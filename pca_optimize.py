...
scores = []
dims = []
neighbor_range = {"start": 2, "end": 50, "gap": 1}
...
for i in range(dim_range["min"], dim_range["max"]):
	...
	for n_neighbors in range(neighbor_range["start"], neighbor_range["end"], neighbor_range["gap"]):
		...
		print("Variance in y_predict: ", variance[int((n_neighbors-neighbor_range["start"])/neighbor_range["gap"])], ", bias: ", bias[int((n_neighbors-neighbor_range["start"])/neighbor_range["gap"])])
		# acc_var_arr.append(gScore**2/mean_variance)
		# print("Accuracy^2 / Variance: ", gScore**2/mean_variance)

	plot_1.xlabel("bias/variance")
	plot_1.ylabel("n_neighbors")
